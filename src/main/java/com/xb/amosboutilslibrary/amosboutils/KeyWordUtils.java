...
public class KeyWordUtils {
     ...
     * @param lyRootID 所在根布局ID
     * @param vID      要滚动到的控件ID
     * @param svID     控件所在ScrollView ID
     * @author gpy
     */
    @SuppressLint("NewApi")
    public static void pullKeywordTop(final Activity activity, final int lyRootID, final int vID,
                                      final int svID) {
        ViewGroup ly = (ViewGroup) activity.findViewById(lyRootID);
        //获取屏幕高度，根据经验，输入法弹出高度一般在屏幕1/3到1/2之间
        final int defaultHeight = ((WindowManager) activity.getSystemService(Context
        ...
        ly.addOnLayoutChangeListener(new View.OnLayoutChangeListener() {
            ...
            public void onLayoutChange(View v, int left, int top, int right,
                ...
                //当高度差大于屏幕1/4，认为是输入法弹出变动，可能会有特殊机型会失败
                if (height > mKeyHeight) {
                    final int lybottom = bottom;
                    //用post防止有时输入法会自动滚动覆盖我们手动滚动
